...
public class MagicProgress extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawThumb(canvas);
        if (TextUtils.equals(textBgStyle, TEXT_BG_STYLE_BUBBLE)) {
            if (isBubbleShow && canMoveThumb) {
                String text = String.valueOf((int) Math.floor((progress - breakProgress) / baseProgress));
                mBubbleView.setFloatOffset(text, thumbX + mDrawingLocation[0], -1);
            }
        } else {
            if (showTextEnable) {
                drawText(canvas);
    ...
    private void showBubble() {
        ...
        managerLayoutParams.y = (mDrawingLocation[1] - mBubbleView.getBubbleHeight()) + (viewHeight / 2)
                - thumbRadios - getContext().getResources().getDimensionPixelSize(R.dimen.dp_px_24);
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (touchOnThumb(event.getX(), event.getY())) {
                    ...
                    showBubble();
                    if (listener != null) {
                        listener.onProgressChangeStarted(progress, false);
                    }
                } else {
                    canMoveThumb = false;
                }
            ...
            case MotionEvent.ACTION_UP:
                if (canMoveThumb) {
                    ...
                    hideBubble();
                    if (listener != null) {
                        listener.onProgressChangeFinish(progress, false);
                    }
                }
                canMoveThumb = false;
                break;
...
//        }
    ...
    public abstract static class OnProgressChangeListener {
        public abstract void onProgressChange(int progress, boolean fromUser);
