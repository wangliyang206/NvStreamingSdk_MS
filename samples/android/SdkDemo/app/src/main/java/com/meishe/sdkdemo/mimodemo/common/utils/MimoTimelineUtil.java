...
public class MimoTimelineUtil {
    ...
    public static NvsTimeline createTimeline() {
        ...
        //timeline.appendAudioTrack(); // 录音轨道 Recording track
    ...
    /**
     * 通过myStory模板重建时间线
     * Rebuild the timeline through the myStory template
     *
     * @param timeline
     */
    public static void rebuildTimelineByTemplate(NvsTimeline timeline) {
        ...
        //Remove all added timeline material, including stickers, subtitles, timeline effects, and composite subtitles
    ...
    private static void addShotVideo(NvsTimeline timeline, MiMoLocalData selectTemplate) {
        ...
        for (int index = 0; index < trackClipCount; index++) {
            ...
            //计算时长比例值 Calculate the duration ratio value
            ...
            //添加主轨道片段 Add the main track fragment
            long inPoint = 0;
            ...
            for (int clipIndex = 0; clipIndex < clipCount; clipIndex++) {
                ...
                long clipInPoint = appendVideoClip(mainVideoTrack,
                        ...
                        trackClipInfo,
                        0, clipCount > 1);
                //记录当前镜头主轨道第一个片段的inPoint值
                // Record the inPoint value of the first segment of the current lens main track
                if (clipIndex == 0) {
                    inPoint = clipInPoint;
                }
            }
            ...
            //添加子轨道片段 Add a subtrack fragment
    ...
    private static void setTimelineData(NvsTimeline timeline, MiMoLocalData selectTemplate) {
        ...
        //添加片头组合字幕 Add title combination subtitles
        ...
        //添加片尾水印 Add an end watermark
        addTimeWaterMark(timeline);

        //添加中间的组合字幕 Add the middle combo subtitles
        long inPoint = 0;
    ...
    private static void setTrackTransition(NvsVideoTrack videoTrack, List<TrackClipInfo> trackClipInfos) {
        ...
        /**
         * 合成时间线转场
         * Synthesize the timeline transition
         */
    ...
    private static void changeSpeed(NvsVideoClip videoClip, TrackClipInfo trackClipInfo) {
        ...
        if (isEqual) {//判断速度是否相同 Determine whether the speed is the same
    ...
    /**
     * 片段编辑页面时间线API
     * Snippet editing page timeline API
     */
    ...
    public static void setTimelineData(NvsTimeline timeline) {
        ...
        //It is important to clone a copy of the music data, because the interface to add a theme deletes the music data
    ...
    /**
     * 重新创建单个clip的TimeLine
     * Recreates the TimeLine of a single clip
     */
    ...
    private static void addVideoClip(NvsVideoTrack videoTrack, ClipInfo clipInfo, boolean isTrimClip) {
        ...
        //会造成第一次播放视频断断续续，需要注释掉
        //The video will be broken the first time, so you need to comment it out
        ...
        int videoType = videoClip.getVideoType();
        if (videoType == NvsVideoClip.VIDEO_CLIP_TYPE_IMAGE) {//当前片段是图片 The current fragment is a picture
            ...
            if (imgDisplayMode == Constants.EDIT_MODE_PHOTO_AREA_DISPLAY) {//区域显示 Area display
            ...
            } else {//全图显示 Full picture display
        ...
        } else {//当前片段是视频 The current clip is video
            ...
            if (!isTrimClip)//如果当前是裁剪页面，不裁剪片段 If the current page is cropped, do not crop the fragment
            {
                return;
            }
            long trimIn = clipInfo.getTrimIn();
    ...
    public static boolean applyTheme(NvsTimeline timeline, String themeId) {
        ...
        //设置主题片头和片尾
        //Set the theme opening and ending
        ...
        // After applying the theme, remove the background music that has already been applied
    ...
    public static boolean buildTimelineMusic(NvsTimeline timeline, List<MusicInfo> musicInfos) {
        ...
        if (musicInfos == null || musicInfos.isEmpty()) {
            ...
            //After removing the music, restore the music from the applied theme
        ...
        //After applying the music, remove the music from the applied theme
        String pre_theme_id = TimelineData.instance().getThemeData();
        if (pre_theme_id != null && !pre_theme_id.isEmpty()) {
    ...
    public static boolean setCaption(NvsTimeline timeline, ArrayList<CaptionInfo> captionArray) {
        ...
        while (deleteCaption != null) {
            ...
            //主题字幕不作删除
            // Subject captions will not be deleted
    ...
    public static boolean setCompoundCaption(NvsTimeline timeline, ArrayList<CompoundCaptionInfo> captionArray) {
        ...
        TimelineData.instance().setCompoundCaptionArray(tempData);
        return true;
    }
    ...
    private static void updateCaptionAttribute(NvsTimelineCaption newCaption, CaptionInfo caption) {
         ...
         *Subtitle StyleUuid needs to be set first before the subtitle properties take effect, because subtitle styles may have their own offset, zoom, rotate, etc.
         *  and the last setting overwrites the previous setting.
         *
         */
        ...
        if (usedShadow == CaptionInfo.ATTRIBUTE_USED_FLAG) {
            ...
            if (isShadow) {
                ...
                newCaption.setShadowOffset(offset);  //字幕阴影偏移量 Subtitle shadow offset
                newCaption.setShadowColor(shadowColor); // 字幕阴影颜色 Subtitle shadow color
            }
        }
