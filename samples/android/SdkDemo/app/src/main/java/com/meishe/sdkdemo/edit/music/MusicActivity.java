...
public class MusicActivity extends BaseActivity {
    ...
    private final int FADE_DURATION = 1000000 * 3;
        ...
        m_musicSingleBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (MainActivity.isUrlResource) {
                    UrlMaterialActivity.nativeUrlMaterial(MusicActivity.this,
                            Constants.ACTIVITY_START_CODE_MUSIC_SINGLE,
                            true, Constants.URL_MATERIAL_MUSIC, false);
                    return;
                }
    ...
    private void haveRecordArea(long cur_audio_inpoint) {
        if (cur_audio_inpoint == -1) {
            ...
            // 距离下一个clip不到1s   Less than 1s from the next clip
            if (next_inpoint != -1 && Math.abs(cur_pos - next_inpoint) < Constants.MUSIC_MIN_DURATION) {
                m_addMusicBtn.setVisibility(View.GONE);
            } else {
                // 视频末尾不到1s  Less than 1 second at the end of the video
                if (Math.abs(cur_pos - m_timeLine.getDuration()) < Constants.MUSIC_MIN_DURATION) {
                    m_addMusicBtn.setVisibility(View.GONE);
                } else {
