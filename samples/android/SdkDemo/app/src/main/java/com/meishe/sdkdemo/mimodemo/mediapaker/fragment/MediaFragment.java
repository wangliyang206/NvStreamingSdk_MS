...
public class MediaFragment extends BaseFragment implements OnTotalNumChange {
     ...
     * The value -1 indicates that the number of videos or pictures is unlimited. If the value is greater than 0, the number of videos or pictures with this value is selected
     */
     ...
     * Displays different media based on the current media type
     */
    private void showLocalMediaByMediaType() {
        //要判断是否有权限，无权限的时候不能去读写SD卡
        //To determine whether you have permission, do not have permission to read or write SD card
    ...
    /**
     * 获取两个List的不同元素
     * Gets different elements of two lists
     * @param adapterList
     * @param listFormList
     * @return
     */
    private List<MediaData> getNeedRefreshList(List<MediaData> adapterList, List<MediaData> listFormList, int from) {
        ...
        //如果数据都和全部内容无关，暂时不做处理
        //If the data is not relevant to the whole content, do not do it for the time being
        if (from != 0 && index != 0) {
            return diff;
        }
