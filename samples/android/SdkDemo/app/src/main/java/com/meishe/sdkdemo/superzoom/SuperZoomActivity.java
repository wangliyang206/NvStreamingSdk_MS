...
import com.meicam.ai.sdk.NvsAIContext;
import com.meicam.ai.sdk.NvsFaceDetector;
...
public class SuperZoomActivity extends BasePermissionActivity implements SurfaceTexture.OnFrameAvailableListener, GLSurfaceView.Renderer {
    ...
    protected int mDetectOrientation = NvsFaceDetector.AI_SDK_DETECTION_FRAME_ROTATION_180;
    ...
    private NvsAIContext mAISdkContext;
    private NvsVideoResolution mCurrentVideoResolution;
    private NvsFaceDetector mNvsFaceDetector;
...
//            "rotate",
    ...
    protected void initViews() {
        mAISdkContext = NvsAIContext.getInstance();
        if (mAISdkContext == null) {
            String licensePath = "assets:/meishesdk.lic";
            mAISdkContext = NvsAIContext.init(getApplicationContext(), licensePath, 0);
        }

        String className = "facemode/ms";
        String faceModelName = MeicamContextWrap.FACE_106_MODEL;
        if (BuildConfig.FACE_MODEL == 240) {
            className = "facemode/ms/240";
            faceModelName = MeicamContextWrap.FACE_240_MODEL;
        }
        ...
        String destModelPath = rootDir + File.separator + className + File.separator + faceModelName;
        boolean copySuccess = FileUtils.copyFileIfNeed(SuperZoomActivity.this, faceModelName, className);
        if (copySuccess) {
            mNvsFaceDetector = NvsFaceDetector.createNvsFaceDetector(destModelPath, NvsFaceDetector.AI_SDK_DETECTION_FRAME_VIDEO_MODE);
        }
        initUI();
        ...
        mOrientationEventListener = new OrientationEventListener(mContext, SensorManager.SENSOR_DELAY_NORMAL) {
            ...
            public void onOrientationChanged(int angle) {
                if (mCameraProxy != null) {
                    int rotation = mCameraProxy.convertDetectOrientation(angle);
                    mDetectOrientation = convertAiRotation(rotation);
                }
            }
        };
    ...
    private int convertAiRotation(int rotation) {
        switch (rotation) {
            case 0:
                return NvsFaceDetector.AI_SDK_DETECTION_FRAME_ROTATION_0;
            case 90:
                return NvsFaceDetector.AI_SDK_DETECTION_FRAME_ROTATION_90;
            case 180:
                return NvsFaceDetector.AI_SDK_DETECTION_FRAME_ROTATION_180;
            case 270:
                return NvsFaceDetector.AI_SDK_DETECTION_FRAME_ROTATION_270;
        }
        return rotation;
    }
        ...
        mButtonRecord.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mZoomFxData != null && !TextUtils.isEmpty(mZoomFxData.localDirPath)) {
                    mNvSuperZoom.setAssetsExternalPath(PathUtils.getSDCardPathByType(NvAsset.ASSET_SUPER_ZOOM));
...
//        });
    ...
    private void detectFace(byte[] data, int width, int height) {
        ...
        List<NvsFaceDetector.NvsFaceData> faceDataList = mNvsFaceDetector.detectBuffer(
                data, width, height, NvsFaceDetector.AI_SDK_DETECTION_BUFFER_FORMAT_RGBA, mDetectOrientation, 0);
        mHumanInfos.clear();
        if (!CommonUtils.isEmpty(faceDataList)) {
            for (NvsFaceDetector.NvsFaceData faceData : faceDataList) {
                if (faceData == null) {
                    continue;
                }
                Rect srcRect = faceData.getFaceRect();
                RectF faceRect = new RectF();

                faceRect.left = srcRect.left;
                faceRect.top = height - srcRect.top;
                faceRect.right = srcRect.right;
                faceRect.bottom = height - srcRect.bottom;
                ...
                //Log.d(TAG, "detectFace: ==========left:" + faceRect.left + "  right:" + faceRect.right + "  top:" + faceRect.top + "  bottom:" +faceRect.bottom + "  mAnchorX:" + mAnchorX + "  mAnchorY" + mAnchorY);
                if (isShowFaceFrame) {
                    List<PointF> position2Ds = faceData.getPointFList();
                    List<PointF> pointList = new ArrayList<>();
                    for (PointF position2D : position2Ds) {
                    ...
                    HumanInfo humanInfo = new HumanInfo(faceRect, pointList, faceData.getPitch(),
                            faceData.getYaw(), faceData.getRoll());
    ...
    protected void onDestroy() {
        mAccelerometer = null;
        if (null != mNvsFaceDetector) {
            mNvsFaceDetector.release();
            mNvsFaceDetector = null;
        }
        /**
         * 释放egl资源
