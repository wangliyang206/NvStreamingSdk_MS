...
public class TrimEditActivity extends BaseEditActivity {
    ...
    /**
     * 选中裁剪的视频索引值
     * Select the clipped video index
     */
    ...
    protected void initEditData() {
        //修改trimIn Modified trimIn
        mTrimIn = mSelectShotVideoInfo.getTrimIn();
        initMultiSequence();
        long trimOut = mTrimIn + mRealNeedDuration;
        //播放时也从trimIn位置开始 It also plays from the trimIn position
    ...
    protected void initListener() {
        ...
        mTimelineEditor.getMultiThumbnailSequenceView().setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (action == MotionEvent.ACTION_UP) {
                    //初始化滑动位置，用于判断是否停止滑动
                    // Initializes the sliding position to determine whether to stop sliding
                    mLastX = mTimelineEditor.getMultiThumbnailSequenceView().getScrollX();
                    mScrollHandler.sendEmptyMessageDelayed(MESSAGE_SCROLLING_FINISH, MESSAGE_DELAY);
                }
                return false;
            }
        ...
        mTimelineEditor.setOnScrollListener(new NvsTimelineEditor.OnScrollChangeListener() {
            ...
            public void onScrollX(long timeStamp) {
                ...
                mTrimIn = trimIn;
                mTrimOutText.setText(TimeFormatUtil.formatMsToString(trimOut));
                mTrimInText.setText(TimeFormatUtil.formatMsToString(trimIn));
        ...
        mMimoVideoFragment.setFragmentLoadFinisedListener(new MimoVideoFragment.OnFragmentLoadFinishedListener() {
            ...
            public void onLoadFinished() {
                mWaitLayout.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        //播放片段，通过片段时间控制
                        //Play the clip, controlled by the clip time
...
//                AppManager.getInstance().finishActivity();
    ...
    private void initMultiSequence() {
        ...
        if (sequenceWidth < innerWith) { //没有填充满，则放大到填充为止 If not filled, zoom in until filled
    ...
    protected NvsTimeline initTimeLine() {
         ...
         * If it is an image, the duration is the last duration, and the thumbnail duration is the last duration, which is the duration of the Timeline
         *If it is a short video, the length is the extended length, and the thumbnail length is the length of the actual video
         *If it is a long video, the length is the extended length, and the length of the thumbnail is the length of the actual long video
         */
        if (mSelectShotVideoInfo == null) {
            return null;
        }
