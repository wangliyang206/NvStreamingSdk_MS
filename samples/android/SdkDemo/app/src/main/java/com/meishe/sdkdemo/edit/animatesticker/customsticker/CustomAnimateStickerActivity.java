...
public class CustomAnimateStickerActivity extends BaseActivity {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.freeMode) {
            ...
            //开启线程存储裁剪的图片
            //Enable thread to store cropped pictures
    ...
    private void saveBitmapToLocal(Bitmap result) {
        ...
        try {
            ...
            //When saving bitmap as file, choose png as the format, otherwise a black background will appear
    ...
    private Bitmap getRectBitmap(String filePath) {
        Bitmap srcBitmap = ImageUtils.getBitmap(filePath);
        int degree = ImageUtils.getRotateDegree(filePath);
        if ((degree != 0) && (null != srcBitmap)) {
            srcBitmap = ImageUtils.rotate(srcBitmap, degree, 0, 0);
        }
        if (srcBitmap == null)
            return null;
    ...
    private void setDisplayImageSize(String imagePath) {
        int statusHeight = ScreenUtils.getStatusBarHeight(this);//状态栏高度 Status bar height
