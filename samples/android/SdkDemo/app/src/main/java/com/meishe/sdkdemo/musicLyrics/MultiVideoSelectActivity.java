...
import com.meishe.sdkdemo.edit.audio.AudioEqualizerActivity;
...
import com.meishe.sdkdemo.sequenceNesting.SequenceNestingActivity;
...
public class MultiVideoSelectActivity extends BaseActivity implements OnTotalNumChangeForActivity {
    ...
    private boolean selectCreateRatio(int makeRatio) {
        ...
        if (fromWhat == Constants.SELECT_VIDEO_FROM_MUSIC_LYRICS) {
            ...
            return true;
        }if (fromWhat == Constants.SELECT_VIDEO_FROM_SEQUENCE_NESTING) {
            AppManager.getInstance( ).jumpActivity(MultiVideoSelectActivity.this, SequenceNestingActivity.class, null);
            return true;
        ...
        }else if(fromWhat == Constants.FROM_MAIN_ACTIVITY_TO_AUDIO_EQUALIZER){
            if (pathList != null && pathList.size( ) >= mLimiteMediaCountMin) {
                AppManager.getInstance( ).jumpActivity(MultiVideoSelectActivity.this, AudioEqualizerActivity.class, null);
                return true;
            }
        }
        return false;
