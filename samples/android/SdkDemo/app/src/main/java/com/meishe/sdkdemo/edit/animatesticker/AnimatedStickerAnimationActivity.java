                ...
                if (!TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerPeriodAnimationPackageId())) {
                    ...
                    if (!TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerInAnimationPackageId())) {
                        if (seekBar.getMaxProgress() - leftValue < IN_OUT_ANIMATION_DEFAULT_DURATION && TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerOutAnimationPackageId())) {
                            ...
                            //If the remaining time is less than the default time of 500 milliseconds after the animation is set (the default time for entering and exiting the animation is 500 ms,
                            // regardless of whether the animation is set or not),
                            // and there is no animation at this time, set the animation time to 0
                    ...
                    if (!TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerOutAnimationPackageId())) {
                        ...
                        //If the remaining time after the animation is set is less than the default time of 500 ms (the default time for entering and exiting the animation is 500 ms,
                        // regardless of whether the animation is set or not),
                        // and there is no animation at this time, the time for entering the animation is set to 0
...
//                            mVideoFragment.changeStickerRectVisible();
    ...
    private void displayAnimationProgress(boolean visible, int type) {
        ...
        if (type == NvAsset.ASSET_ANIMATED_STICKER_IN_ANIMATION) {
            if (TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerInAnimationPackageId())) {
                ...
                //1.判断是否有出动画 无则重置
                //Judge whether there is out animation, reset if not
                if (TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerOutAnimationPackageId())) {
                    seekBar.reset();
    ...
    private void checkInit() {
        if (curAnimatedSticker != null) {
            ...
            //Select an item resource to complete in Fragment
            if (!TextUtils.isEmpty(curAnimatedSticker.getAnimatedStickerPeriodAnimationPackageId())) {
