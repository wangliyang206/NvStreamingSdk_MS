...
public class ThemeShootUtil {
    ...
    /**
     * 获取主题拍摄在assets中的路径
     * Gets the path of the subject shot in assets
     *
     * @return
     */
...
//        return themeModelList;
    ...
    private static void computeNeedTime(ThemeModel themeModel) {
        ...
        for (ThemeModel.ShotInfo shotInfo : shotInfos) {
            ...
            //是否是空镜头 Whether it is empty or not
            ...
            //变速区间需要的时长
            // The length of the shift interval
            long speedTime = 0L;
            //变速区间对应原录制时长
            // The variable speed interval corresponds to the original recording duration
            long oriTime = 0;
            for (ThemeModel.ShotInfo.SpeedInfo speedInfo : speed) {
                long needDuration = (long) ((Long.parseLong(speedInfo.getEnd()) - Long.parseLong(speedInfo.getStart())) *
                        (speedInfo.getSpeed0() + speedInfo.getSpeed1()) / 2);
...
//        }
     ...
     * @return
     */
    public static String get9V21PathByPath(String filePath) {
        if (!TextUtils.isEmpty(filePath) && !filePath.contains("9v21.")) {
            String[] split = filePath.split("\\.");
            StringBuffer stringBuffer = new StringBuffer();
            for (int i = 0; i < split.length; i++) {
                if (i == split.length - 1) {
                    stringBuffer.append("9v21.").append(split[i]);
                } else {
                    stringBuffer.append(split[i]);
                }
            }
    ...
    public static int getClipCountFromSpeed(List<ThemeModel.ShotInfo.SpeedInfo> speeds, long duration) {
        ...
        if (speeds != null) {
            for (int i = 0; i < speeds.size(); i++) {
                ...
                //添加原视频 Add original video
                if (start < speedStart) {
                    count++;
                    start += speedStart;
                }
                //添加变速视频 Add variable speed video
                count++;
