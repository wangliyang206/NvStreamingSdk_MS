...
public class SelectMusicActivity extends BaseActivity {
    ...
    //    private String mAssetsPath = "sound";
    private String mAssetsPath = "music";
    ...
    private int mDouVideoMinDuration = 15000000;
    ...
    protected void initData() {
        ...
        if (intent != null) {
            ...
            if (bundle != null) {
                ...
                hasSelectMusic = bundle.getBoolean(Constants.SELECT_MUSIC_HAVE, true);
            }
        }
        if (!hasSelectMusic) {
            mNoMusicBtn.setEnabled(false);
            mNoMusicBtn.setTextColor(getResources().getColor(R.color.fx_un_select));
        } else {
            mNoMusicBtn.setTextColor(getResources().getColor(R.color.ms_blue));
        }
...
//        });
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                final List<MusicInfo> audioInfoList = mAudioUtil.listMusicFilesFromAssets(SelectMusicActivity.this, mAssetsPath);
    ...
    public void playMusic(MusicInfo musicInfo, boolean is_local) {
        ...
        if (mPlayMusic.getDuration() == 0) {
            ToastUtil.showToast(mContext, getResources().getString(R.string.audio_duration_error));
            return;
        }
        ...
        if (mPlayMusic.isPlay()) {
            ...
            mCutViewLayout.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mFromWhatPage == Constants.SELECT_MUSIC_FROM_DOUVIDEO) {
                        mSelectMusicTime.setText("00:00/" + duration_text);
                        mCutMusicView.setRightHandleVisiable(false);
                        mCutMusicView.setMinDuration(mDouVideoMinDuration);
                    } else {
    ...
    private void closeMusicActivity() {
        ...
        if (mPlayMusic != null) {
            if (mFromWhatPage == Constants.SELECT_MUSIC_FROM_DOUVIDEO) {
                if (mPlayMusic.getDuration() - mPlayMusic.getTrimIn() <= mDouVideoMinDuration) {
                    mPlayMusic.setTrimOut(mPlayMusic.getDuration() - mPlayMusic.getTrimIn());
                } else {
                    mPlayMusic.setTrimOut(mPlayMusic.getTrimIn() + mDouVideoMinDuration);
                }
            }
        }
