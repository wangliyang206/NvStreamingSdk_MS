...
public class SelectBottomMenu extends BaseCustomView {
        ...
        mNextButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!mMenuAdapter.isFull()) {
                    Toast.makeText(mContext, mContext.getResources().getString(R.string.please_add_material), Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    public void addClipPath(String path, long duration) {
        ...
        if (lastVisibleItemPosition < mMenuAdapter.mCurrentSelectPosition + 1) {
            mSelectMenu.smoothScrollBy(ScreenUtils.dip2px(mContext, ITEM_WIDTH_DP * 2), 0);
        }
    }
    ...
    private int findPositionInCurTime(long curTime) {
        if (mIntervalPoints == null) {
            //首次获取进度间隔点 Get the progress interval point for the first time
            mIntervalPoints = getIntervalPoints();
        }
        //从左向右对比，用于正常播放或用户正向拖动进度条
        //Compare the progress bar from left to right for normal playback or for the user to drag the progress bar forward
        if (mIntervalPoints.size() > mMenuAdapter.mCurrentSelectPosition + 1) {
            if (curTime >= mIntervalPoints.get(mMenuAdapter.mCurrentSelectPosition + 1)) {
        ...
        if (mIntervalPoints.size() > mMenuAdapter.mCurrentSelectPosition + 1) {
            //从右向左对比，用于用户反向拖动进度条
            // Compare from right to left for the user to drag the progress bar backward
            if (curTime <= mIntervalPoints.get(mMenuAdapter.mCurrentSelectPosition)) {
                return mMenuAdapter.mCurrentSelectPosition - 1;
            }
        }
     ...
     * The point in time at which the editable video clip is obtained, excluding the default video clip
     *
     * @return
     */
